...
export default function Footer() {
  ...
  const handleEmailClick = (e) => {
    // Only redirect if the email field is empty
    if (!email) {
      e.preventDefault();
      router.push('/login');
    }
  ...
  const footerLinks = [
    {
      title: "Resources",
      items: [
        { name: "Free Articles", href: "#" },
        { name: "Habit Worksheets", href: "#" },
        { name: "Reading Guide", href: "#" },
    ...
    <footer className="bg-gray-800 text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
          <div className="md:col-span-2">
            ...
            <p className="text-gray-300 mb-6">
              The proven framework for building good habits and breaking bad ones.
            </p>
            
            <div className="mb-6">
              <h4 className="text-sm font-semibold mb-2 uppercase tracking-wider">Subscribe to our newsletter</h4>
              {subscribed ? (
                ...
                <form onSubmit={handleSubscribe} className="flex">
                  <input
                    ...
                    className="bg-gray-700 text-white px-4 py-2 rounded-l-md focus:outline-none focus:ring-2 focus:ring-blue-500 w-full cursor-pointer"
                    required
                  />
                  <button
                    type="submit"
              ...
              {socialLinks.map((social) => (
                ...
                >
                  ...
                  {hoveredItem === social.name && (
                    <span className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-gray-700 text-white text-xs px-2 py-1 rounded-md whitespace-nowrap">
                      {social.name}
                    </span>
                  )}
                </Link>
        ...
        <div className="mt-12 border-t border-gray-700 pt-6 text-center text-gray-400 text-sm">
          &copy; {new Date().getFullYear()} Atomic Habits. All rights reserved.
        </div>
      </div>
    </footer>
  );
